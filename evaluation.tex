...
US balancing-authority traces are drawn from the EIA's hourly electric-power monitor \cite{eia_data}, European transmission-system-operator traces from the ENTSO-E Transparency Platform \cite{entsoe_data}, and weather observations from the NSF NCAR Research Data Archive for Atmospheric and Ocean Sciences \cite{ncep_gfs_2015}.
...
The complete list of regions, along with their identifying balancing-authority or TSO code, mean carbon intensity, coefficient of variation, dominant generation source, and aggregate renewable share, is given in Table~\ref{tab:eval-regions} in Appendix~\ref{chap:appendix}.

Carbon intensity is computed as the demand-weighted average of source-level intensities using the IPCC AR5 operational carbon factors \cite{ipcc_carbon_factors_2014} introduced in \S2.1.2, expressed in $\text{g}\cdot\text{CO}_2\text{eq}/\text{kWh}$ at hourly granularity.
...
The standalone forecast-quality comparison in \S\ref{sec:eval-forecast-quality} shows that LiteCast consistently achieves higher concordance.
